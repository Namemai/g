...
settings = {
    ...
    "message":"บัญชีนี้ถูกป้องกันโดย by,Mai ระบบได้บล็อคคุณอัตโนมัติ!",
    "comment":"""ระบบออโต้ BY:Mai
╔══════════════┓
╠ รับติดตั้งบอทวี10 
╠ไอดีไลน์ mai06555mai
╠ลิ้งไอดี
╠http://line.me/ti/p/~mai06555mai
╠เบอติดต่อ 0655581505
╚══════════════┛""",
    "userAgent": [
...
╠★►Me ↔คอนแทค
╠★►H1 ↔คำสั่งใช้ในกลุ่ม
╠★►H2 ↔คำสั่งตั้งค่าเชลล์
╠★►H3 ↔ คำสั่งโซเชียลมีเดีย
╠★►H4 ↔ คำสั่งให้สิริพูด
╠★►H5 ↔ คำสั่งแปลภาษา
╠★►บอทออน ↔ การใช้งานบอท
╠★►แทค ↔ แทคคนทั้งห้อง
...
#==============================================================================#
                    ...
                    line.sendMessage(to, "ВΌŦ ÓŅLÍŇỀ\n {}".format(str(runtime)))
...
#==============================================================================#
                    ...
                    msg.contentType = 13
                    line.sendMessage(to, "มันตายหมดแล้ว (｡◕‿◕｡)")
                    line.sendContact(to, "")                
#==============================================================================#
...
#==============================================================================#
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                               ...
                               mi_d = Mids[:33]
                               line.createGroup("ВΌŦ",mi_d)
                               line.sendText(msg.to,"[ВΌŦ LÍŇỀ]")
                               line.createGroup("ВΌŦ",mi_d)
                               line.sendText(msg.to,"[ВΌŦ LÍŇỀ]")
                               line.createGroup("ВΌŦ",mi_d)
                               line.sendText(msg.to,"ВΌŦ LÍŇỀ]")
                    ...
                    if spl[0] == "":
                        ...
                        line.updateProfile(prof)
                        line.sendText(msg.to,"สำเร็จแล้ว")
                elif "มุด on" in msg.text.lower():
                    spl = re.split("off",msg.text,flags=re.IGNORECASE)
                    if spl[0] == "":
                        try:
                            gid = spl[1].split(" ")[0]
...
#==============================================================================#
                ...
                elif msg.text in ["แทค2 on"]:
                    settings["delayMention"] = True
                    line.sendMessage(msg.to,"❋เปิดแทค คนแทคกลับ")
                
                elif msg.text in ["แทค2 off"]:
                    settings["delayMention"] = False
                  ...
                  if msg._from in admin:
                     ...
                     if spl in [""," ","\n",None]:
                         line.sendMessage(msg.to, "ตั้งข้อความคนคนลบสมาชิดเรียบร้อย")
                          ...
                          settings["kick"] = spl
                          line.sendMessage(msg.to, "nตั้งค่าข้อความเมื่อมีคนลบสมาชิกแล้ว\nดังนี้👇\n\n👉{}".format(str(spl)))
...
#=============COMMAND PROTECT=========================#
                ...
                elif msg.text.lower() == 'เปิดหมด on':
                    if RfuProtect["inviteprotect"] == True:
                        if settings["lang"] == "JP":
                ...
                elif msg.text.lower() == 'ปิดหมด off':
                    if RfuProtect["inviteprotect"] == False:
                        if settings["lang"] == "JP":
